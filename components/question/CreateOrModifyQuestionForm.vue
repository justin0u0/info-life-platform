...
export default {
  name: 'QuestionCreateOrModifyQuestionForm',
  components: {
    CreateOrModifyEditor,
  },
  props: {
  ...
  methods: {
    async preGetTags() {
      const { data } = await getTags({ filter: { type: 'question' }, sort: { _id: 1 } });
      this.tags = data;
    },
    setContent(content) {
      console.log('[QuestionCreateOrModifyQuestionForm:updateContent]');
